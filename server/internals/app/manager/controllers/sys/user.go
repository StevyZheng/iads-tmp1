...
import (
	...
	models "iads/server/internals/pkg/models/common"
	"iads/server/internals/pkg/models/sys"
	"iads/server/pkg/cache"
	"iads/server/pkg/convert"
	"iads/server/pkg/hash"
	"iads/server/pkg/jwt"
...
func (User) AdminsRoleIDList(c *gin.Context) {
	...
	roleList := []uint64{}
	where := sys.UserRole{UserID: adminsid}
...
func (User) Login(c *gin.Context) {
	...
	where := sys.User{UserName: username, Password: password}
	user := sys.User{}
...
func (User) EditPwd(c *gin.Context) {
	...
	modelNew := sys.User{Password: new_password}
